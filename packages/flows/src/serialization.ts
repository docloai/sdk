...
import type { NodeDef, VLMProvider, OCRProvider, JSONSchemaNode, FlowContext, FlowInput } from "@docloai/core";
import { createFlow, type FlowOptions, type Flow, type BuiltFlow } from './flow-builder.js';
import { parse, extract, split, categorize, trigger, output } from '@docloai/nodes';
...
export function extractNodeMetadata(node: NodeDef<unknown, unknown>): { nodeType: string; config: NodeConfig } | null {
  // Nodes created with the @docloai/nodes functions don't expose config
