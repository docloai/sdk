...
export async function buildProviderFromConfig(
  ...
  if (config.type === 'vlm') {
    ...
    try {
      ...
      const module: DynamicModuleExports = await import(/* webpackIgnore: true */ '@docloai/providers-llm');
      ...
      if (!createVLMProvider) {
        throw new Error('@docloai/providers-llm does not export createVLMProvider');
      }
      ...
      throw new Error(
        ...
        `Make sure @docloai/providers-llm is installed.`
      );
    }
    ...
    try {
      // @ts-ignore - Dynamic import, package may not be installed
      const module: DynamicModuleExports = await import(/* webpackIgnore: true */ '@docloai/providers-datalab');
      ...
      if (config.provider === 'surya') {
        const suryaProvider = (module.suryaProvider || module.default?.suryaProvider) as
          ...
          | undefined;
        if (!suryaProvider) {
          throw new Error('@docloai/providers-datalab does not export suryaProvider');
        }
        ...
        const markerProvider = (module.markerProvider || module.default?.markerProvider) as
          ...
          | undefined;
        if (!markerProvider) {
          throw new Error('@docloai/providers-datalab does not export markerProvider');
        }
      ...
      throw new Error(
        ...
        `Make sure @docloai/providers-datalab is installed.`
      );
    }
  } else {
    // This branch is unreachable due to discriminated union, but TypeScript doesn't know that
