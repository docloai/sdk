...
import { safeJsonParse } from "@docloai/core/security";
import type {
  ObservabilityConfig,
...
import { PROMPT_REGISTRY, renderPrompt } from "@docloai/prompts";
import { SCHEMA_REGISTRY } from "@docloai/schemas";
...
async function runWithConsensus<T>(
  ...
  ctx?: import('@docloai/core/internal/validation-utils').NodeCtx,
  ...
  const allRunMetrics: import('@docloai/core/internal/validation-utils').StepMetric[][] = [];
  ...
  const captureMetricsForRun = async <R>(runFn: () => Promise<R>): Promise<{ result: R; metrics: import('@docloai/core/internal/validation-utils').StepMetric[] }> => {
    const capturedMetrics: import('@docloai/core/internal/validation-utils').StepMetric[] = [];
    ...
    if (ctx?.metrics) {
      ...
      ctx.metrics.push = (metric: import('@docloai/core/internal/validation-utils').StepMetric) => {
  ...
  if (parallel) {
    ...
    const runPromises = Array.from({ length: config.runs }, async (_, i) => {
      ...
      let lastMetrics: import('@docloai/core/internal/validation-utils').StepMetric[] = [];
      let attempts = 0;
